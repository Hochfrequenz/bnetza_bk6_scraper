...
def _synthetic_proceeding_page(az: str) -> str:
    ...
    pdf = "https://www.bundesnetzagentur.de/DE/Beschlusskammern/1_GZ/BK6-GZ/2023/" f"{az}/{az}_dok.pdf"
    return (
        "<html><head><base href='/'/></head><body><div id='content'>"
        f"<h2>Titel {az}</h2>"
...
async def test_mirror_processes_multiple_proceedings(tmp_path: Path) -> None:
    ...
    az_two = "BK6-23-002"
    proc_one_url = (
        "https://www.bundesnetzagentur.de/DE/Beschlusskammern/1_GZ/BK6-GZ/2023/" f"{az_one}/{az_one}_konsultation.html"
    )
    proc_two_url = (
        "https://www.bundesnetzagentur.de/DE/Beschlusskammern/1_GZ/BK6-GZ/2023/" f"{az_two}/{az_two}_konsultation.html"
    )
    pdf_one = "https://www.bundesnetzagentur.de/DE/Beschlusskammern/1_GZ/BK6-GZ/2023/" f"{az_one}/{az_one}_dok.pdf"
    pdf_two = "https://www.bundesnetzagentur.de/DE/Beschlusskammern/1_GZ/BK6-GZ/2023/" f"{az_two}/{az_two}_dok.pdf"
    lv_html = (
        f'<html><body><a href="{proc_one_url}">{az_one}</a>' f'<a href="{proc_two_url}">{az_two}</a></body></html>'
    )
    with aioresponses() as mocked:
        mocked.get(LV, status=200, body=lv_html)
        mocked.get(AV, status=200, body="<html><body></body></html>")
...
async def test_mirror_skips_index_link_without_aktenzeichen(tmp_path: Path) -> None:
    ...
    lv_html = f'<html><body><a href="{PROC_URL}">BK6-23-241</a>' f'<a href="{bad_url}">Übersicht 2023</a></body></html>'
    with aioresponses() as mocked:
        mocked.get(LV, status=200, body=lv_html)
        mocked.get(AV, status=200, body="<html><body></body></html>")
        mocked.get(PROC_URL, status=200, body=proc_html)
