...
PDF_X = "https://www.bundesnetzagentur.de/DE/Beschlusskammern/1_GZ/BK6-GZ/2023/BK6-23-241/BK6-23-241_x.pdf"
PDF_Y = "https://www.bundesnetzagentur.de/DE/Beschlusskammern/1_GZ/BK6-GZ/2023/BK6-23-241/BK6-23-241_y.pdf"
...
def _synthetic_proceeding_page(az: str) -> str:
    ...
    pdf = f"https://www.bundesnetzagentur.de/DE/Beschlusskammern/1_GZ/BK6-GZ/2023/{az}/{az}_dok.pdf"
    return (
        "<html><head><base href='/'/></head><body><div id='content'>"
        f"<h2>Titel {az}</h2>"
...
async def test_mirror_processes_multiple_proceedings(tmp_path: Path) -> None:
    ...
    az_two = "BK6-23-002"
    proc_one_url = (
        f"https://www.bundesnetzagentur.de/DE/Beschlusskammern/1_GZ/BK6-GZ/2023/{az_one}/{az_one}_konsultation.html"
    )
    proc_two_url = (
        f"https://www.bundesnetzagentur.de/DE/Beschlusskammern/1_GZ/BK6-GZ/2023/{az_two}/{az_two}_konsultation.html"
    )
    pdf_one = f"https://www.bundesnetzagentur.de/DE/Beschlusskammern/1_GZ/BK6-GZ/2023/{az_one}/{az_one}_dok.pdf"
    pdf_two = f"https://www.bundesnetzagentur.de/DE/Beschlusskammern/1_GZ/BK6-GZ/2023/{az_two}/{az_two}_dok.pdf"
    lv_html = f'<html><body><a href="{proc_one_url}">{az_one}</a><a href="{proc_two_url}">{az_two}</a></body></html>'
    with aioresponses() as mocked:
        mocked.get(LV, status=200, body=lv_html)
        mocked.get(AV, status=200, body="<html><body></body></html>")
...
async def test_mirror_min_year_skips_old_proceedings(tmp_path: Path) -> None:
    old_url = (
        "https://www.bundesnetzagentur.de/DE/Beschlusskammern/1_GZ/BK6-GZ/2018/BK6-18-001/BK6-18-001_konsultation.html"
    )
    new_url = (
        "https://www.bundesnetzagentur.de/DE/Beschlusskammern/1_GZ/BK6-GZ/2023/BK6-23-001/BK6-23-001_konsultation.html"
    )
...
async def test_mirror_skips_index_link_without_aktenzeichen(tmp_path: Path) -> None:
    ...
    lv_html = f'<html><body><a href="{PROC_URL}">BK6-23-241</a><a href="{bad_url}">Übersicht 2023</a></body></html>'
    with aioresponses() as mocked:
        mocked.get(LV, status=200, body=lv_html)
        mocked.get(AV, status=200, body="<html><body></body></html>")
        mocked.get(PROC_URL, status=200, body=proc_html)
