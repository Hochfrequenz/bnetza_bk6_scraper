...
# One PDF under a proceeding (has Aktenzeichen), the PID list as both PDF and Excel (no
# Aktenzeichen), and one HTML sub-link.
PAGE_HTML = (
    ...
    "<a href='/DE/Beschlusskammern/BK06/BK6_83_Zug_Mess/835_mitteilungen_datenformate/"
    "Mitteilung_48/Anlagen/PID_3_2_info.xlsx?__blob=publicationFile&v=1'>Prüfidentifikatoren (Excel)</a>"
    "<a href='/DE/Beschlusskammern/BK06/BK6_83_Zug_Mess/831_gpke/detail.html'>Mehr</a>"
    "</div></body></html>"
)


def test_parse_candidate_documents_extracts_documents_with_context() -> None:
    cands = parse_candidate_documents(PAGE_HTML, page_url=GPKE_PAGE)
    by_name = {c.filename: c for c in cands}
    assert set(by_name) == {"BK6-24-174_GPKE_Teil1_Lesefassung.pdf", "PID_3_2.pdf", "PID_3_2_info.xlsx"}
    ...
    # The Excel PID list is captured alongside the PDF — this is the whole point.
    pid_xlsx = by_name["PID_3_2_info.xlsx"]
    assert pid_xlsx.aktenzeichen is None
    assert pid_xlsx.title == "Prüfidentifikatoren (Excel)"
    assert "__blob=publicationFile" in pid_xlsx.source_url
...
def test_parse_followable_links_returns_absolute_html_links() -> None:
    ...
    # Document anchors (PDF, Excel) are not followable HTML links — they must never be
    # enqueued and fetched as text by the crawler.
    assert not any(".pdf" in link.lower() or ".xlsx" in link.lower() for link in links)
